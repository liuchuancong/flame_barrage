...
class BarrageEngine extends FlameGame {
  BarrageEngine({required this.config, required this.emojiAtlas}) {
    _parser = RichParser(atlas: emojiAtlas);
    _layout = MixedLayout(atlas: emojiAtlas);
  ...
  final BarragePool _pool = BarragePool(maxSize: 150);
  ...
  Future<void> onLoad() async {
    await super.onLoad();
    _trackManager.initialize(config, size.y);
    _initialized = true;
  }
  ...
  void pushMessage(BarrageItem item) {
    if (item.content.length > config.maxTextLength) return;

    if (_waiting.length > config.dangerousQueueSize) {
  ...
  void update(double dt) {
    ...
    _massiveMode = _waiting.length > config.massiveModeThreshold;

    _emitTimer += dt;
    final interval = _massiveMode ? config.massiveEmitInterval : config.normalEmitInterval;
  ...
  void _dispatchWaiting() {
    ...
    final activeComponents = children.whereType<BarrageComponent>();
    if (activeComponents.length >= config.maxVisibleCount) return;

    final item = _waiting.first;
    final fragments = _parser.parse(item.content);
    final layoutResult = _layout.layout(fragments, config: config);
    ...
    double startY = trackIndex * config.trackHeight + (config.trackHeight - layoutResult.height) / 2;

    if (item.type != BarrageType.scroll) {
      track.locked = true;
      startX = (size.x - layoutResult.width) / 2;
      if (item.type == BarrageType.bottomFixed) {
        startY = size.y - (trackIndex + 1) * config.trackHeight + (config.trackHeight - layoutResult.height) / 2;
      }
    }
    ...
    mockEntry.speed = item.type == BarrageType.scroll
        ? _speedStrategy.calculate(mockEntry, size.x, config, targetTrack: track, massiveMode: _massiveMode)
        : 0.0;

    final component = _pool.obtain(
      entry: mockEntry,
      picture: picture,
      fixedDuration: Duration(milliseconds: config.fixedDurationMs),
    );
  ...
  void _updateTrackMetrics() {
    final now = DateTime.now().millisecondsSinceEpoch;
    final activeComponents = children.whereType<BarrageComponent>().toList();
    ...
    for (final track in _trackManager.tracks) {
      ...
      final compLen = activeComponents.length;
      for (int i = 0; i < compLen; i++) {
        final comp = activeComponents[i];
      ...
      if (youngestComp != null) {
        ...
        if (track.locked && now > track.lastLaunchTime + config.fixedDurationMs) {
          track.locked = false;
        }
      }
    }

    final activeLen = activeComponents.length;
